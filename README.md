...
能,全程零成本、零维护。所有计算都在 GitHub 的机器上:

- **电脑关机、卸载 Python、退出 GitHub / Server酱 网页登录,都不影响** —— 云端用的是仓库里加密保存的 SendKey,不依赖你浏览器的登录状态
- **代码放一年不动照跑** —— 每 10 分钟自动执行一次,不依赖任何本地程序
- **不会被 GitHub 自动停掉** —— GitHub 会对 60 天没有任何提交的仓库停用定时任务,所以脚本**每天会自动提交一次状态文件**(心跳),让仓库始终有活动
- **数据源挂了自动换源** —— 新浪 → 腾讯 → Yahoo 三级容灾,单个源故障无感
- **真出故障会主动告诉你** —— 连续 30 分钟取不到任何行情时,会推一条「⚠️ 监控异常」到微信,恢复正常后自动清零。不会出现"你以为它在盯着,其实早坏了"
- **依赖版本已锁主版本**(`requests<3`、`PyYAML<7`),避免几年后依赖大版本升级把脚本跑挂

唯一建议你做的(可选):每年想起来的时候去 Actions 页面跑一次测试推送,确认微信还收得到。万一收不到,按下面「常见问题」换一个 SendKey 就行。
